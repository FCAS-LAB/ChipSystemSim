...
cleanup() { "$root/scripts/provision_dind_swarm.sh" --nodes "$nodes" --prefix "$prefix" --dind-data-root "$out/dind-storage" --cleanup || true; }
trap cleanup EXIT
...
"$root/scripts/provision_dind_swarm.sh" --nodes "$nodes" --dind-image chipsystemsim:dind-netem --image "$image" --image-archive "$archive" --total-cpus "$total_cpus" --total-memory "${total_memory_gib}g" --delay 1ms --rate none --image-load-jobs 4 --dind-data-root "$out/dind-storage" --prefix "$prefix"
...
"$root/scripts/run_dind_mlp_dp.sh" --nodes "$nodes" --prefix "$prefix" --config-dir "$out/config/mlp-dp-nodes${nodes}" --stack "mlp_steady_n${nodes}" --output-dir "$out/measurement" --timeout-seconds 1600
