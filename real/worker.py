...
async def handle(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, run_root: Path) -> None:
    ...
    try:
        ...
        if not isinstance(staged_files, list):
            raise ValueError("staged_files must be a list")
        ...
        # Phase two PopNet consumes bench.txt generated by the coordinator's
        # completed phase one.  Transfer only explicitly declared regular
        # files, into this isolated work directory, before executing PopNet.
        # This preserves upstream's phase ordering without a shared host mount.
        for staged_file in staged_files:
            if not isinstance(staged_file, dict):
                raise ValueError("each staged file must be an object")
            destination = staged_file.get("destination")
            encoded_data = staged_file.get("data")
            if not isinstance(destination, str) or Path(destination).name != destination:
                raise ValueError("staged file destination must be a plain filename")
            if not isinstance(encoded_data, str):
                raise ValueError("staged file data must be base64 text")
            data = base64.b64decode(encoded_data, validate=True)
            if len(data) > 64 * 1024 * 1024:
                raise ValueError("staged file exceeds the 64 MiB transfer limit")
            (workdir / destination).write_bytes(data)
